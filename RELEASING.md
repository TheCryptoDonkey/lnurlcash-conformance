# Releasing lnurlcash-conformance
...
```bash
npm dist-tag rm lnurlcash-conformance next
npm deprecate lnurlcash-conformance@0.1.0-next.0 "bootstrap seed - use 0.1.0 or later"
```
...
2. Add a GitHub Actions publisher:
   - Repository: `TheCryptoDonkey/lnurlcash-conformance`
...
The frozen-vector gate matters most here, because the vectors *are* the
product: the release regenerates them and fails if a single byte differs from
what is committed, so a published version can never disagree with the vectors
it shipped. `prepublishOnly` runs the same `npm run check` locally before
